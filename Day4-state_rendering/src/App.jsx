...
const App = () => {
    ...
    const deleteProduct = (id) => {
      const products = productsData.filter((product) => product.id !== id);
      setProductsData(products);
    }

    return (
        <div>
           <h1>State is now rendering</h1>
          <div className="flex flex-wrap gap-3">
            {productsData.map((product)=>{
              return <ProductsCard key={product.id} product={product} del={deleteProduct} />
            })}
          </div>
       </div>
    );
